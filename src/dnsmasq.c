...
extern struct hash_struct *users_by_name;
extern struct hash_struct *users_by_id;

extern struct hash_struct *users_by_name_204;
extern struct hash_struct *users_by_id_204;

extern struct hash_struct *users_by_name_cna;
extern struct hash_struct *users_by_id_cna;
...
static void async_event(int pipe, time_t now)
{
    ...
    if (read_event(pipe, &ev, &msg))
        switch (ev.event) {
        case EVENT_RELOAD:
            clr_user(&users_by_id, &users_by_name);
            clr_user(&users_by_id_204, &users_by_name_204);
            clr_user(&users_by_id_cna, &users_by_name_cna);
            system("clear_ndsWG.sh");
#ifdef HAVE_DNSSEC
        ...
        case EVENT_DUMP:
            if (daemon->port != 0) {
                dump_cache(now);
                print_users_by_table(users_by_id, users_by_name);
                print_users_by_table(users_by_id_cna, users_by_name_cna);
                print_users_by_table(users_by_id_204, users_by_name_204);
            }
            break;
