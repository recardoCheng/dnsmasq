...
#define LOPT_WILDCARD_FILE 345
#define LOPT_HTTPS_WILDCARD_FILE 346
#define LOPT_INDEX 347
#define LOPT_204_WILDCARD_FILE 348
#define LOPT_CNA_WILDCARD_FILE 349
...
#endif
{
    ...
    { "wildcard-file", 1, 0, LOPT_WILDCARD_FILE },
    { "https-wildcard-file", 1, 0, LOPT_HTTPS_WILDCARD_FILE },
    { "wildcard-204-file", 1, 0, LOPT_204_WILDCARD_FILE },
    { "wildcard-cna-file", 1, 0, LOPT_CNA_WILDCARD_FILE },
    { "index", 1, 0, LOPT_INDEX },
    { "mx-host", 1, 0, 'm' },
    { "mx-target", 1, 0, 't' },
    { "cache-size", 2, 0, 'c' },
    { "port", 1, 0, 'p' },
...
static int one_opt(int option, char *arg, char *errstr, char *gen_err, int command_line, int servers_only)
{
    ...
    case LOPT_INDEX:
        if (!atoi_check(arg, &daemon->index))
            ret_err(gen_err);
        break;

    case LOPT_204_WILDCARD_FILE:
        daemon->wildcard_204file = opt_string_alloc(arg);
        if (daemon->wildcard_204file) {
            FILE *f = fopen(daemon->wildcard_204file, "r");
            if (!f) {
                my_syslog(LOG_ERR, "Unable to read 204 wildcard file");
                break;
            } else {
                char line[MAX_WILDCARD_LENGTH];
                memset(line, 0, sizeof(line));

                while (fgets(line, sizeof(line), f)) {
                    if  (daemon->wildcard_204domain[daemon->numwc204]) {
                        strncpy(daemon->wildcard_204domain[daemon->numwc204], line, strlen(line)-1);
                        daemon->numwc204++;
                        if (daemon->numwc204>=MAX_WILDCARD_ENTRY) {
                            printf("maximum 204 wild card entry\n");
                            break;
                        }
                    } else
                        break;
    ...
    case LOPT_CNA_WILDCARD_FILE:
        daemon->wildcard_cnafile = opt_string_alloc(arg);
        if (daemon->wildcard_cnafile) {
            FILE *f = fopen(daemon->wildcard_cnafile, "r");
            if (!f) {
                my_syslog(LOG_ERR, "Unable to read cna wildcard file");
                break;
            } else {
                char line[MAX_WILDCARD_LENGTH];
                memset(line, 0, sizeof(line));
                ...
                while (fgets(line, sizeof(line), f)) {
                    if  (daemon->wildcard_cnadomain[daemon->numwccna]) {
                        strncpy(daemon->wildcard_cnadomain[daemon->numwccna], line, strlen(line)-1);
                        daemon->numwccna++;
                        if (daemon->numwccna>=MAX_WILDCARD_ENTRY) {
                            printf("maximum cna wild card entry\n");
                            break;
                        }
                    } else
                        break;
    ...
    case LOPT_HTTPS_WILDCARD_FILE:
        daemon->https_wildcard_file = opt_string_alloc(arg);
        if (daemon->https_wildcard_file) {
            FILE *f = fopen(daemon->https_wildcard_file, "r");
            if (!f) {
                my_syslog(LOG_ERR, "Unable to read https wildcard file");
                break;
            } else {
                char line[MAX_WILDCARD_LENGTH];
                memset(line, 0, sizeof(line));
                ...
                while (fgets(line, sizeof(line), f)) {
                    if  (daemon->https_wildcard_domain[daemon->https_numwc]) {
                        strncpy(daemon->https_wildcard_domain[daemon->https_numwc], line, strlen(line)-1);
                        daemon->https_numwc++;
                        if (daemon->https_numwc>=MAX_WILDCARD_ENTRY) {
                            printf("maximum https wild card entry\n");
                            break;
                        }
                    } else
                        break;
    ...
    case LOPT_WILDCARD_FILE:
        daemon->wildcard_file = opt_string_alloc(arg);
        if (daemon->wildcard_file) {
            FILE *f = fopen(daemon->wildcard_file, "r");
            if (!f) {
                my_syslog(LOG_ERR, "Unable to read wildcard file");
                break;
            } else {
                char line[MAX_WILDCARD_LENGTH];
                memset(line, 0, sizeof(line));
                ...
                while (fgets(line, sizeof(line), f)) {
                    if  (daemon->wildcard_domain[daemon->numwc]) {
                        strncpy(daemon->wildcard_domain[daemon->numwc], line, strlen(line)-1);
                        daemon->numwc++;
                        if (daemon->numwc>=MAX_WILDCARD_ENTRY) {
                            printf("maximum wild card entry\n");
                            break;
                        }
                    } else
                        break;
