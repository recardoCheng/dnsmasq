...
struct hash_struct *users_by_name = NULL;
struct hash_struct *users_by_id = NULL;

struct hash_struct *users_by_name_204 = NULL;
struct hash_struct *users_by_id_204 = NULL;

struct hash_struct *users_by_name_cna = NULL;
struct hash_struct *users_by_id_cna = NULL;

const char delimiters[] = ".";

unsigned int gid=1;
unsigned int gid_204=1;
unsigned int gid_cna=1;
...
int do_cnacmp(char* domain_name)
{
  unsigned int i=0;

  for (i=0; i< daemon->numwccna; i++) {
    if (wildcmp(daemon->wildcard_cnadomain[i], domain_name)) {
      return 1;
    }
  }
  return 0;
...
int do_204cmp(char* domain_name)
{
  unsigned int i=0;

  for (i=0; i< daemon->numwc204; i++) {
    if (wildcmp(daemon->wildcard_204domain[i], domain_name)) {
      return 1;
    }
  }
  return 0;
...
int do_wildcmp(char* domain_name)
{
  unsigned int i=0;

  for (i=0; i< daemon->numwc; i++) {
    if (wildcmp(daemon->wildcard_domain[i], domain_name)) {
      return 1;
    }
  }
  return 0;
...
int do_https_wildcmp(char* domain_name)
{
    unsigned int i=0;

    for (i=0; i< daemon->https_numwc; i++) {
        if (wildcmp(daemon->https_wildcard_domain[i], domain_name)) {
            return 1;
        }
    }
    return 0;
...
cname_loop:
            ...
            // "/tmp/iiot/script/nds_wg.sh" @iNodeProA/iNodeProEagle
            // "/usr/sbin/dns_reply.sh" @iNodeOne_FE/iNodePro
            char reply_addr[768]= {0};
            char reply_cmd[1024]="/usr/sbin/dns_reply.sh ";

            // "/tmp/iiot/script/nds_204.sh" @iNodeProA/iNodeProEagle
            // "/usr/sbin/dns_204.sh" @iNodeOne_FE/iNodePro
            char reply_204_addr[768]= {0};
            char reply_204_cmd[1024]="/usr/sbin/dns_204.sh ";

            // "/tmp/iiot/script/nds_cna.sh" @iNodeProA/iNodeProEagle
            // "/usr/sbin/dns_cna.sh" @iNodeOne_FE/iNodePro
            char reply_cna_addr[768]= {0};
            char reply_cna_cmd[1024]="/usr/sbin/dns_cna.sh ";
...
cname_loop1:
            ...
            for (j = ntohs(header->ancount); j != 0; j--) {
                ...
                if (aqclass == C_IN && res != 2 && (aqtype == T_CNAME || aqtype == qtype)) {
                    if (aqtype == T_CNAME) {
                        ...
                        //Recardo
                        if (!hit204) {
                            hit204=do_204cmp(name);
                        }
                        if (!hitcna) {
                            hitcna=do_cnacmp(name);
                        }
                        if (!dnshit) {
                            dnshit=do_wildcmp(name);
                            if (!dnshit) {
                              if (!https_hit) {
                                https_hit=do_https_wildcmp(name);
                              }
                            }
                        }
                        ...
                        if (flags & (F_IPV4 | F_IPV6 | F_CNAME)) {
                            ...
                            //whitelist
                            HASH_FIND(hh2, users_by_name, ip, strlen(ip), s);
                            if (!s) {
                                char tmp[768]= {0};
                                if (!dnshit) {
                                    dnshit=do_wildcmp(name);
                                ...
                                if (dnshit || https_hit) {
                                    add_user_by_table(gid++, ip, &users_by_id, &users_by_name);
                                }
                                if (strlen(reply_addr)) {
                                    snprintf(tmp, sizeof(tmp), "%s%s", " ", ip);
                                } else {
                                    snprintf(tmp, sizeof(tmp), "%d %d %s", daemon->index, https_hit, ip);
                                }
                                int catlen=sizeof(reply_addr)-strlen(reply_addr);
                                strncat(reply_addr, tmp, catlen-1);
                                docmd=1;
                            }

                            //204 block
                            s=NULL;
                            HASH_FIND(hh2, users_by_name_204, ip, strlen(ip), s);
                            if (!s) {
                                char tmp[768]= {0};
                                if (!hit204) {
                                    hit204=do_204cmp(name);
                                ...
                                if (hit204) {
                                    add_user_by_table(gid_204++, ip, &users_by_id_204, &users_by_name_204);
                                }
                                if (strlen(reply_204_addr)) {
                                    snprintf(tmp, sizeof(tmp), "%s%s", " ", ip);
                                } else {
                                    snprintf(tmp, sizeof(tmp), "%d %s", daemon->index, ip);
                                }
                                int catlen=sizeof(reply_204_addr)-strlen(reply_204_addr);
                                strncat(reply_204_addr, tmp, catlen-1);
                                do204cmd=1;
                            }

                            //cna allow
                            s=NULL;
                            HASH_FIND(hh2, users_by_name_cna, ip, strlen(ip), s);
                            if (!s) {
                                char tmp[768]= {0};
                                if (!hitcna) {
                                    hitcna=do_cnacmp(name);
                                ...
                                if (hitcna) {
                                    add_user_by_table(gid_cna++, ip, &users_by_id_cna, &users_by_name_cna);
                                }
                                if (strlen(reply_cna_addr)) {
                                    snprintf(tmp, sizeof(tmp), "%s%s", " ", ip);
                                } else {
                                    snprintf(tmp, sizeof(tmp), "%d %s", daemon->index, ip);
                                }
                                int catlen=sizeof(reply_cna_addr)-strlen(reply_cna_addr);
                                strncat(reply_cna_addr, tmp, catlen-1);
                                docnacmd=1;
                            }
                        }
            ...
            if ((dnshit || https_hit) && docmd) {
                int catlen=sizeof(reply_cmd)-strlen(reply_cmd);
                if (dnshit) {
                  my_syslog(LOG_INFO, "allow address :%s %s", name, reply_addr);
                }
                else {
                  my_syslog(LOG_INFO, "allow https address :%s %s", name, reply_addr);
                }
                strncat(reply_cmd, reply_addr, catlen-1);
                my_syslog(LOG_INFO, "cmd:%s", reply_cmd);
                system(reply_cmd);
            }

            if (hit204 && do204cmd) {
                int catlen=sizeof(reply_204_cmd)-strlen(reply_204_cmd);
                if (hit204) {
                  my_syslog(LOG_INFO, "block 204 address :%s %s", name, reply_204_addr);
                }
                strncat(reply_204_cmd, reply_204_addr, catlen-1);
                my_syslog(LOG_INFO, "204 cmd:%s", reply_204_cmd);
                system(reply_204_cmd);
            }

            if (hitcna && docnacmd) {
                int catlen=sizeof(reply_cna_cmd)-strlen(reply_cna_cmd);
                if (hitcna) {
                  my_syslog(LOG_INFO, "block cna address :%s %s", name, reply_cna_addr);
                }
                strncat(reply_cna_cmd, reply_cna_addr, catlen-1);
                my_syslog(LOG_INFO, "cna cmd:%s", reply_cna_cmd);
                system(reply_cna_cmd);
            }
                    ...
                    if (crecp) {
                        ...
                        do {
                            ...
                            if (crecp->flags & F_CNAME) {
                                ...
                                //recardo
                                if (!dnshit) {
                                    dnshit=do_wildcmp(cname_target);
                                    if (!dnshit) {
                                      if (!https_hit) {
                                        https_hit=do_https_wildcmp(cname_target);
                                      }
                                    }
                                }
                                //204 block
                                if (!hit204) {
                                  hit204=do_204cmp(cname_target);
                                }

                                //cna allow
                                if (!hitcna) {
                                  hitcna=do_cnacmp(cname_target);
                                }
                            ...
                            if (crecp->flags & F_NEG) {
                                ...
                                ans = 1;
                                if (!dryrun) {
                                    char *ip=inet_ntoa(crecp->addr.addr.addr.addr4);
                        ...
                        if (docmd) {
                            int catlen=sizeof(reply_cmd)-strlen(reply_cmd);
                            my_syslog(LOG_INFO, "ansreq allow address :%s %s", name, reply_addr);
                            strncat(reply_cmd, reply_addr, catlen-1);
                            my_syslog(LOG_INFO, "ansreq cmd:%s", reply_cmd);
                            system(reply_cmd);
                        }
                        if (do204cmd) {
                            int catlen=sizeof(reply_204_cmd)-strlen(reply_204_cmd);
                            my_syslog(LOG_INFO, "block 204 address :%s %s", name, reply_204_addr);
                            strncat(reply_204_cmd, reply_204_addr, catlen-1);
                            my_syslog(LOG_INFO, "block 204 cmd:%s", reply_204_cmd);
                            system(reply_204_cmd);
                        }
                        if (docnacmd) {
                            int catlen=sizeof(reply_cna_cmd)-strlen(reply_cna_cmd);
                            my_syslog(LOG_INFO, "allow cna address :%s %s", name, reply_cna_addr);
                            strncat(reply_cna_cmd, reply_cna_addr, catlen-1);
                            my_syslog(LOG_INFO, "allow cna cmd:%s", reply_cna_cmd);
                            system(reply_cna_cmd);
                        }
