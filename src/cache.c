...
arp_get(const char req_ip[])
{
    ...
    memset(daemon->dns_req_mac, 0x0, sizeof(daemon->dns_req_mac));
    /* Skip first line */
    while (!feof(proc) && fgetc(proc) != '\n');

    /* Find ip, copy mac in reply */
    reply = NULL;
    while (!feof(proc) && (fscanf(proc, " %15[0-9.] %*s %*s %17[A-Fa-f0-9:] %*s %*s", ip, daemon->dns_req_mac) == 2)) {
        if (strcmp(ip, req_ip) == 0) {
            my_syslog(LOG_INFO, "mac:%s", daemon->dns_req_mac);
            break;
        }
    }
